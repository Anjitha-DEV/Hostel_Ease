...
from .models import District, Hostel, Application, Mess, Visitor
from .forms import StudentRegistrationForm, WardenRegistrationForm, ApplicationForm, HostelRegistrationForm, UserProfileForm, MessForm, VisitorForm
...
def apply_visitor(request):
    ...
    hostel = Hostel.objects.filter(warden__user=student).first() or Hostel.objects.filter(applications__student=student).first()
